...
let computerPicked = document.getElementsByClassName("computerPicked")[0]; //change border-color  and image
let yourPicked = document.getElementsByClassName("yourPicked")[0]; //change border-color  and image
let whoWin = document.getElementsByClassName("whoWin")[0]; // YOU WIN || YOU LOST || TIE UP
let Against = document.getElementsByClassName("Against")[0]; // Hide During Tie
let playAgain = document.getElementsByClassName("playAgain")[0]; // Come to first Screen with Updated Score
let yourAura = document.getElementsByClassName("yourAura")[0];
let pcAura = document.getElementsByClassName("pcAura")[0];
let ChooseDiv = document.getElementsByClassName("ChooseDiv")[0]; //hide while showing result
let ResultDiv = document.getElementsByClassName("ResultDiv")[0]; // hide while next game
...
nextBtn.addEventListener("click", () => {
  const trophyImage = congratulationScreen.querySelector("img");

  if (MyScore > ComputerScore) { // Player wins
    congratulationScreen.classList.remove("hidden");
    congratulationScreen.querySelector(".message").innerText = "HURRAY!! YOU WON THE GAME"; // Update text to show win message
    trophyImage.classList.remove("hidden");
  } 
  else if (MyScore = ComputerScore) { // Player wins
    congratulationScreen.classList.remove("hidden");
    congratulationScreen.querySelector(".message").innerText = "IT'S A TIE! GREAT MATCH!!! "; // Update text to show win message
    trophyImage.classList.add("hidden");
  } 
  else { // Player loses
    congratulationScreen.classList.remove("hidden");
    congratulationScreen.querySelector(".message").innerText = "UH OH! BETTER LUCK NEXT TIME"; // Update text to show lose message
    trophyImage.classList.add("hidden");
  }
});
...
let showResult = (UserValue, ComputerValue) => {
  ...
  switch (UserValue) {
    ...
    case "paper":
      ...
      break;
    default:
      alert("Something Went Wrong Refresh the website");
  }
  if (Winner === "user") {
